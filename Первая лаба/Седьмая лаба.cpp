...
public:
    ...
    void Draw(const glm::mat4& view, const glm::mat4& projection, const glm::vec3& viewPos,
        float xPos, float yPos, float rot, bool isXMoving, bool isYMoving, bool isRotating)
    {
        ...
        for (auto& info : meshes) {
            ...
            // Подсветка движущихся частей
            if (info.name == "Куб.002" && isXMoving) {
                float pulse = (sin(glfwGetTime() * 15.0f) + 1.0f) / 2.0f;
                glUniform3f(glGetUniformLocation(shaderProgram, "material.diffuse"),
                    1.0f, 0.2f + pulse * 0.3f, 0.2f);
            }
            else if (info.name == "Куб.003" && isYMoving) {
                float pulse = (sin(glfwGetTime() * 15.0f + 2.0f) + 1.0f) / 2.0f;
                glUniform3f(glGetUniformLocation(shaderProgram, "material.diffuse"),
                    1.0f, 0.2f + pulse * 0.3f, 0.2f);
            }
            else if (info.name == "Куб.004" || info.name == "Куб.001") {
                if (isRotating) {
                    float pulse = (sin(glfwGetTime() * 15.0f + 4.0f) + 1.0f) / 2.0f;
                    glUniform3f(glGetUniformLocation(shaderProgram, "material.diffuse"),
                        0.8f, 0.8f, 0.2f + pulse * 0.3f);
                }
                else {
...
int main()
{
    ...
    cout << "\n========== УПРАВЛЕНИЕ ==========\n";
    cout << "ДВИЖЕНИЕ КУБ.002:        R / F\n";
    cout << "ДВИЖЕНИЕ КУБ.003:   T / G\n";
    cout << "ВРАЩЕНИЕ (барабан + шпиндель):          Q / E\n";
    cout << "КАМЕРА:                                 WASD + МЫШЬ\n";
    cout << "==============================================\n\n";
    cout << "ВИЗУАЛЬНЫЙ ЭФФЕКТ: движущиеся части меняют цвет\n\n";
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) { xPos -= speed; movingX = true; }
        if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS) { xPos += speed; movingX = true; }
        xPos = max(xMin, min(xMax, xPos));

        if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS) { yPos -= speed; movingY = true; }
        if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS) { yPos += speed; movingY = true; }
        yPos = max(yMin, min(yMax, yPos));

        if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) { rot += rotSpeed; isRotating = true; }
        if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) { rot -= rotSpeed; isRotating = true; }
        ...
        myModel.Draw(view, projection, cameraPos, xPos, yPos, rot, movingX, movingY, isRotating);
